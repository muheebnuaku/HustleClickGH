...
export default function OrgProjects() {
  ...
  const create = async () => {
    ...
    if (cost > wallet) { setErr(`This project needs ${formatCurrency(cost)} but your wallet has ${formatCurrency(wallet)}. Add funds first.`); return; }
    setBusy(true);
    try {
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-zinc-600 dark:text-zinc-400 mt-1">Wallet: <strong className="text-emerald-600">{formatCurrency(wallet)}</strong></p>
          </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="text-sm font-medium">Reward per approved (GH₵) *</label><Input type="number" step="0.5" value={form.reward} onChange={(e) => setForm({ ...form, reward: e.target.value })} placeholder="2.00" /></div>
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="text-sm text-zinc-500 mt-1">{p.counts.approved}/{p.maxSubmissions} collected · {p.counts.pending} pending · {formatCurrency(p.reward)}/each</p>
                      </div>
                    ...
                    <p className="text-xs text-zinc-400 mt-1.5">Budget {formatCurrency(p.budget)} · spent {formatCurrency(p.spent)}</p>
                  </CardContent>
                </Card>
              </Link>
            ))}
